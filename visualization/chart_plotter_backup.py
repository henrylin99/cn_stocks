...
class ChartPlotter:
    ...
    def create_kline_chart(self, df: pd.DataFrame, ts_code: str,
                          ...
                          width: int = 1200, height: int = 800) -> go.Figure:
        """
        创建K线图表
        Args:
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        marker=dict(
                            ...
                            size=15 + confidence * 10,  # 根据置信度调整大小
    ...
    def create_stock_analysis_chart(self, ts_code: str, strategy_names: Optional[List[str]] = None,
        ...
        try:
            ...
            with StockAnalyzer() as analyzer:
                ...
                if df.empty:
                    return self._create_empty_chart(f"股票 {ts_code} 无数据", 1200, 800)
            ...
            print(f"创建图表失败: {e}")
            return self._create_empty_chart(f"创建图表失败: {str(e)}", 1200, 800)
